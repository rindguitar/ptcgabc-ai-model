...
- **§45 の横展開監査（ユーザー指摘）**: replay の steps/action を読む全7スクリプトを確認し
  off-by-one は**ゼロ**（ペアリングする4本は全て `steps[i+1]`・残り3本は action 非依存 or
  初手デッキ提出走査のみ）。ただし replay-format.md の樹形図が同一 step ペアと誤読できる
  記述だったため、t+1 規則の明記＋正実装再利用の指示を追記（再発防止）。
