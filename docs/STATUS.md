...
## 次の一手（優先順）
0. **【撤回・訂正】「誤デプロイ」は誤報＝§43 は本番で完璧に動作**（2026-07-19）:
   正しいペアリング（replay の obs[t] への応答は **steps[t+1].action**・teacher-extract 等の
   既存スクリプトは元から正実装）で再測定した結果、**v4 発火 14/14・v3.5 発火 17/17（100%）**。
   v3 ベースラインは 8%（23/287）＝§43 の効果は明確。誤報の原因は Claude のアドホックな
   プローブが同一 step でペアにした off-by-one（教訓: 行動ログ解析は既存スクリプトの
   ペアリング実装を再利用する）。再アップロード不要。
   **初日の実勢**: 発火は完璧だが**発火機会そのものが希少**（v4 14/64・v3.5 17/62 試合）。
   律速は札の可用性＝60枚中1枚（サイド落ち~10%・エンジンの掘りで捨てる/埋まる）。
   deck-out は試合比 26%（v3）→ 15-16%（v4/v3.5）に減少。勝率 v4 0.460 / v3.5 0.525（各1日・判定続行）。
...
3. **遅滞デッキ評価済み（2026-07-19・過去最良候補）**: 65c6b47e 平均0.730/実メタ頻度加重0.763・
   88cc3fe1 0.668/0.686（いずれも champion best 0.631 超え・nn+floor8＝§43 リサイクル入り操縦）。
   ミラー除外の index を補正した苦手分析: **両者共通の天敵は a8c57d4b（我々の旧 ismcts 旗艦デッキ・
   実メタ出現わずか0.9%）**で 0.05/0.20。対して実メタ33%の a4066acd（エンジン型）には 0.85/0.70
   ＝§41「遅滞はエンジンを食う」の定量確認。65c6 の残る弱点は同型遅滞 88cc（4.7%）に 0.30。
   **追試済み（40試合・計60試合/相手）: 平均 0.730→0.727 で再現・確証**。計60の合算で
   単純平均 0.728・頻度加重 0.748。弱点構造も安定（a8c57d4b 0.15/出現0.9%・同型遅滞 88cc
   0.35/4.7%・c921b9b2 0.52/8.7%）。実メタ33%の a4066acd には 0.82。
   **次**: ①v4/v3.5 の判定後、負けた枠を「65c6b47e×勝った閾値の操縦」に差し替えるのが有力
   ②gate の最悪ケース基準は 0.9% の天敵に引きずられる＝頻度加重平均の併記を検討（要議論）。
