...
## 次の一手（優先順）
0. **⚠️ v4/v3.5 の再アップロードが必要（誤デプロイ判明・2026-07-19）**: 初日 126 episode の
   行動ログ解析で、**両枠とも §43 の強制手が発火していない**（機会32回中発火3回＝v3 の 7% と同水準・
   指紋も v3 と一致）。切り分け: ①手元 tar は正しい（tar 内コードに失敗局面を食わせると [13]＝
   リサイクルを返す・recycle_at=25 も同梱確認）②2枠は別エージェント（episode 重複0・セルフマッチ2件）
   →結論: **アップロードされた実体が旧 v3 相当**（同名類似ファイルの取り違いの可能性大）。
   正: models/submission_alphago_v4.tar.gz (1,838,220B / sha256 f829f944…)・
   _v35.tar.gz (1,838,318B / sha256 10fd55aa…)。初日データは A/B として無効・再アップ後に仕切り直し。
