...
## ゲームのルール用語（PTCG・一般ルール）

> ここに書くのは**一般的なゲームルール**（カード名・効果文＝Pokémon Elements は書かない）。
> モデルはこれらの機構を数値特徴として学ぶ（[features](#features特徴量--encoding)・design-decisions §16）。

### サイド（prize cards）
ゲーム開始時に各プレイヤーが伏せる 6 枚。相手ポケモンを**きぜつ（KO）させると規定枚数を取る**。
**先に全部取った側が勝ち**。だから「サイドを何枚進めるか／取られるか」が勝敗の直接の物差し
（NN の value も観測のサイド残数からこれを学ぶ）。

### ex / megaEx / tera / aceSpec
高性能カードの種別（エンジンのフラグ）。**サイド価値**に効くのは前二つ:
- **ex**: KO されると相手が**サイド2枚**取る（強力な代わりにリスク大）。
- **megaEx**: KO されると**サイド3枚**（さらにハイリスク）。
- **tera**: 別メカニクス＝**ベンチにいる間はワザのダメージを受けない**（サイド枚数とは無関係。tera ex は
  ex フラグ由来で2枚）。
- **aceSpec**: **デッキに1枚だけ**入れられる制限（対戦中のサイドとは無関係＝構築ルール）。

### 弱点 / 抵抗（weakness / resistance）
- **弱点**: 攻撃側のタイプが防御側の弱点と一致すると**ダメージが2倍**（本プロジェクトの近似）。
- **抵抗**: 一致すると**ダメージが軽減**（標準 −30 で近似）。
- 「倒せるか（KO 判定）」は基礎ダメージでなく、**弱点/抵抗を織り込んだ有効ダメージ**で判断すべき。

### 有効ダメージ / サイド価値・被弾スレット（特徴）
モデルに入れた派生特徴（design-decisions §16）:
- **有効ダメージ**: `基礎 ×(弱点なら2) −(抵抗なら30)`。KO 判定・ダメージ比・サイド報酬の土台。
- **サイド価値（攻め）**: 「この手で相手を KO ＝サイド何枚取れるか」を行動特徴に明示（ex 2枚取りを学べる）。
- **被弾スレット（守り）**: 「相手の最大ワザで自分が受ける有効ダメージ・KO されるか・失うサイド」を状態
  特徴に明示。攻めだけでなく**守り（逃げる/守る判断）を対称に**学ばせる（features v2.2）。
