...
### ミラー評価の罠（mirror-eval pitfall）
**同一デッキ・単一相手のミラー対戦で net/操縦を比較すると、差を過大/過小評価する**落とし穴（§25 訂正）。
`eval_net.py` の NN vs ISMCTS ミラーで operative が replay-tuned に 0.675>>0.500 と大差に見えたが、外部基準
（実メタ16デッキ・非ミラー・eval-deck）で測り直すと**同点**だった。原因は非中立リファレンス＋注入等との
相互作用。共適応ロック（deck×pilot）の net 版で、**破り方は同じ＝外部基準（実メタ相手プール・実戦A/B）で
測る**。net/操縦の確定判断は eval-deck vs 実メタで行い、ミラーは大まかな確認に留める。
