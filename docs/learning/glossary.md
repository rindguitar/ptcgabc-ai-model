...
`Q + c * sqrt(ln(N)/n)`。
**記号**: `Q`＝その手の平均価値（勝率の実績・0〜1）、`n`＝**その手**を試した回数、
`N`＝**親局面**で全ての手を試した合計回数、`c`＝探索の強さ（大きいほど未知の手を試す）。
第2項は「n が小さい手ほど大きくなるボーナス」＝あまり試していない手を押し上げる。

### PUCT（Predictor + UCT）
UCT に **事前確率（priors）** を掛けた版。AlphaZero が使う。`Q + c_puct * P(a) * sqrt(N)/(1+n)`。
**記号**: `Q`/`n`/`N` は UCT と同じ。`P(a)`＝**手 a の事前確率**（NN の policy head が出す「この手は
筋が良さそう」の度合い・合計1）、`c_puct`＝探索の強さ。
priors を掛けるので、**まだ試していない手の中でも NN が推す手から先に探索される**＝少ない試行で強くなる。
...
相手の可視札を証拠に候補 d を `weight(d)=exp(-β·misses)` で重み付け比例サンプリングする。
**記号**: `d`＝相手デッキの候補1つ、`misses`＝そのデッキでは**説明できない可視札の枚数**
（相手が実際に使った札のうち d に入っていない数＝矛盾の量）、`β`＝矛盾へのペナルティの強さ。
`exp(-β·misses)` は「矛盾が多い候補ほど重みが指数的に小さくなる」＝
**miss 0 の候補が最も選ばれ、miss が増えるほど急速に見捨てられる**という意味。可視札が増えるほど分布が真アーキタイプへ尖る＝**belief sharpening**。従来のハード整合
...
**注意（floor 抑圧・§33）**: floor の基準は「我々の heuristic の判断」なので、**分布の違う借り物方策**
（例: 1位のクローン）が学んだ見慣れない良い手を凡手へ差し戻すことがある（実測 floor0=0.681 > floor8=0.588）。
自前系譜の net には守り・クローン系には足枷＝クローン評価/提出時は floor を弱める A/B を添える。

### 蒸留（distillation）/ behavioral cloning（行動クローン）
**強い教師の選択を真似る**ようにネットを教師あり学習すること。教師は2系統:
- **ISMCTS 蒸留**（自前教師・対局を自分で生成）: 安定した土台作り。天井＝教師の強さ。
- **実戦教師のクローン（§33）**: リーダーボード replay から上位チームの (局面→選択) を one-hot で学ぶ。
  教師対局を生成するコストがゼロで、教師は自前 ISMCTS より強い可能性。進捗指標は **top-1 一致率**
  （検証局面で教師と同じ手を最尤にできた率。例: 0.166→0.470）。`make teacher-extract / teacher-tune`。
どちらも欠点は**教師を超えられない**こと（天井＝教師）と、データ外の局面（OOD）で無防備なこと。

### AlphaGo 型 pilot（クローン priors × 接地ロールアウト / leaf_rollouts）
複合誤差（下記）への正対（§38）。クローン net を**操縦そのものにせず「探索の事前分布」に降格**し、
決定化 PUCT（＝ISMCTS の骨格）の手の順序付けだけを任せ、**葉の価値は接地ロールアウト**（終局まで
実地プレイ）で評価する。AlphaGo (2016) の SL policy priors＋rollout evaluation と同構成。
模倣の良い部分（プロらしい候補手）を残し、無案内な局面は探索が矯正する。
`make_nn_mcts_agent(leaf_rollouts=1)`。**「葉を ISMCTS で評価」は入れ子探索＝同じ計算の非効率な重複**
（内側の木は使い捨て・外側が育たない）——MCTS は訪問の多い葉を自動で深く展開するので、
「有望な所だけ深く」は1本の木で構造的に起きる。[nn_mcts.py](../../src/nn_mcts.py)。

### 行動クローンの複合誤差（compounding errors / covariate shift）
クローンは1手ごとに教師とズレる（一致率0.44）→ 自分のミスが「教師の見たことのない局面」を作る →
そこでは無案内で更にミス、の雪だるま（模倣学習で DAgger が発明された理由）。長い決定連鎖
（エンジンデッキ＝100〜150決定）で最悪化する。実測: 2教師のクローンが同型失敗（§37・本人より
30手短い試合でデッキ切れ14倍＝ペース配分と変換が再現できない）。**対策はクローンを操縦から
「探索の事前分布」へ降格**——候補手の順序付けだけ模倣し、評価と矯正は接地探索に任せる。
...
- **soft-π**: MCTS の**訪問回数分布**をそのまま使う（例 `[0.6, 0.3, 0.1]`）。情報が多いが、探索が浅いと
  分布が**平坦**になり無情報になる。
- **温度 `τ`**: `π ∝ visits^(1/τ)` で両者を1本化。
  **記号**: `visits`＝MCTS が各手を訪問した回数、`π`＝学習ターゲットの確率分布、`∝`＝「比例する」
  （計算後に合計1へ正規化する）。`1/τ` 乗するので、**τ が小さいほど訪問数の差が強調され**
  （`τ→0` で最多訪問だけが 1＝one-hot）、`τ=1` なら訪問数そのままの比率になる。
  本プロジェクトは既定 `τ=0`（one-hot・安全）、深い教師にしたとき `τ>0` で soft を解放する。

### cross-entropy（交差エントロピー）/ BCE
- policy の損失＝**交差エントロピー** `-(π * log softmax(logits)).sum()`。
  **記号**: `logits`＝net が出す各手の生スコア、`softmax(logits)`＝それを確率に変換したもの＝**net の予測**、
  `π`＝**正解の分布**（教師の手）、`.sum()`＝全ての手について足す。
  正解の手ほど `π` が大きく、その手の予測確率が低いと `log` が大きな負値になる＝
  **「正解に低い確率を与えたら大きく罰する」**という意味。soft な π もそのまま使える。
...
- **EMA（Exponential Moving Average・指数移動平均）**: `θ_ema ← d·θ_ema + (1−d)·θ`（d≈0.99）。
  **記号**: `θ`（シータ）＝net の重み（今回の学習結果）、`θ_ema`＝平均版の重み、
  `d`＝**古い平均をどれだけ残すか**（decay）、`←`＝「左辺を右辺で置き換える」。
  d=0.99 なら「99% は今までの平均・1% だけ今回の重み」＝**新しい値を少しずつ混ぜて滑らかにする**。
  毎 iter 更新し、推論/保存には θ_ema を使う。
...
`P(s,a) ← (1−ε)·p + ε·η`、`η〜Dir(α)`（本プロジェクトは ε=0.25・α=10/手数）。
**記号**: `s`＝局面、`a`＝手、`P(s,a)`＝その局面でその手に与える事前確率、`p`＝net が出した元の priors、
`η`（イータ）＝**乱数で作った確率分布**、`η〜Dir(α)` は「η をディリクレ分布から引く」の意味
（α が小さいほど極端に偏った乱数になる）、`ε`（イプシロン）＝**乱数を混ぜる割合**。
ε=0.25 なら「net の予測 75% ＋ 乱数 25%」の配合。ネットの priors が
...
例: `v' = clamp01(v + α×(自駒数−敵駒数)/5)` を挟んだ floored NN を 40 試合評価。
**記号**: `v`＝net が出した元の勝率予測（0〜1）、`v'`＝補正後の値、`α`＝**補正の強さ**（0 なら無補正）、
`/5` ＝駒数差を 0〜1 程度の大きさに揃えるための割り算、`clamp01`＝はみ出した値を 0〜1 に丸める関数。
つまり「**盤面の駒が相手より多ければ勝率予測を少し上げる**」という手作りの補正。効けば「この信号を
...
### ネットデッキ（netdecking）
自作探索でデッキを作る代わりに、**実環境の淘汰を生き残ったデッキをそのまま借りる**こと（§29b）。
何万試合のメタ選抜を無料で利用できる＝低計算戦略と整合。ただし**デッキは持ち主の方策と共適応**して
いるので、借りた側の操縦で性能が出るとは限らない（1位デッキの実測: 本人0.708・我々の操縦0.537）。
→ 操縦もセットで借りる＝行動クローン（§33）とペアで考える。

### エンジン回転数（エンジンループ）
エンジン型デッキの強さの本体（§40）。「特性（ドロー）→手札が増える→たね/進化を張る→特性源が増える→…」
の自己増幅ループが1ターンに何回転するか。行動差分の実測では1位は我々の**1.4倍**の総アクション
（特性 8.6 vs 4.0 回/試合が起点）で、ベンチ数は同じ＝**密度でなく速度**の差。回転の burst に対して
「山札を戻して回し続ける」リサイクル（§39・`CardMeta.is_deck_recycle`・残デッキ≤15で最優先プレイ）が
対になる。個別ルールの差を1つずつ潰すより、このループ構造を見た方が診断が速い。

### 両席操縦交絡（eval-deck の測定限界）
`eval-deck` は**相手デッキも同じ net が操縦**する（factory が両席に同一評価器を配る）。そのため
net をまたいだデッキスコア比較は「相手をどれだけ上手く操縦したか」が混入し、厳密には揃わない
（ミラー評価の罠の親戚・§33）。実ラダーでは相手は他人の bot＝この交絡は消えるので、**僅差の判定は
実戦 A/B に委ねる**。※実測では結論を覆すほどではなかった（12h 中間がローカルと同順・§34）。
...
勝率は試行回数が少ないとブレる。標準誤差 `SE ≈ sqrt(p(1-p)/N)`。
**記号**: `p`＝測定された勝率（0〜1）、`N`＝試合数、`SE`＝**その勝率がどれくらいブレるか**の目安幅。
N が分母なので**試合数を4倍にしてブレは半分**（sqrt のため）。20試合だと ±0.1 程度ブレる
...
結果を見る**前に**「何をどう分類し・各結果でどう動くか」を書き残す作法（§34 で実践: teacher の負け方を
①OOD大ポカ→floor4 ②ライン切れ→継ぎ足し ③相性→撤退、と事前分類）。データを見てから基準を作ると
**後知恵バイアス**（都合よく解釈して撤退が遅れる/早まる）が入るのを防ぐ。§21「判定基準の事前確定」の一般形。

### フィールドスカウト / 敗因研究モード
replay コーパスに登場する**全チーム**の行動指紋（勝率・主デッキのロール構成・思考時間・特性回数・
初攻ターン・リサイクル率・敗北時獲得サイド）を1パスで集計する（§41・[scout_field.py](../../scripts/scout_field.py)）。
`--vs TEAM --result loss` で「TEAM が負けた試合の相手側」だけを抽出＝**その相手の倒し方**をデータで学ぶ
（例: 1位の敗北70試合 → スタール型が消耗戦で寝かせる、が有効と判明）。episode には両席の観測・行動が
入っているので、DL した対局の**相手側もすべて**解析できる。

### アーキタイプ（型）と実行度
フィールドの「型」は**デッキのロール構成**で決まり（例: エンジン型 poke19/sup15/ene7・中速加速型
ene10-13/加速3・スタール型 poke12-13/初攻遅）、観測される行動（特性回数・リサイクル率）は**デッキが
要求する打ち回し×操縦の実行度**の積。同じデッキでも実行度は違う（実測: 同一デッキで本人特性8.6 vs
我々4.0・§40）＝型はデッキが決め、**操縦は型を「どれだけ体現できるか」**を決める。共適応ロックの
フィールド版の見方。純粋に操縦だけの属性は思考時間くらい（3秒でも135秒でも勝者はいる・§41）。

### 行動差分レポート（behavior diff）
2チームの replay から操縦の**測れる次元を全部並べて一度に比較**する手法（§40・`make` 不要の
[behavior_diff.py](../../scripts/behavior_diff.py)）。次元: MAIN 行動の内訳/試合・ペース（初攻撃・
サイド取得・deckCount 軌跡・リサイクル率）・盤面（ベンチ数）・サーチ取得の中身（フェッチ優先）。
**同一デッキ同士で比較**すれば共適応の交絡を最小化できる。モグラ叩き（1差分ずつ修正）と違い、
単発の異常値でなく**構造（ループ）**が見える——§40 では11次元を並べて「エンジン回転数」1本に集約できた。
改善のたびに同じ定規で本人との距離を再測定する。
...
## 対戦の基礎用語（指標・コードに出てくるもの）

コードとログに頻出する対戦の基本概念。**カード名・ワザ名・効果文は書かない**（規約）ので、
すべて機構と数値の言葉で説明する。カードを指すときは `cardId`（例: id756）で参照する。

### サイド（prize）/ 献上サイド / `prize_value`
相手のポケモンを**きぜつ（KO）させると取れる得点札**。規定枚数を先に取り切ると勝ち。
1体あたり何枚取られるかは札ごとに違い、コードでは `prize_value: cardId -> 1/2/3`
（[cards.py](../../src/cards.py)）。**献上サイド**＝自分が相手に与えてしまったサイド枚数
（本プロジェクトの主要な失点指標・「〜/戦」＝1試合あたりの回数で測る）。
**サイド3の札を前に置いたまま殴られる**のが §80 で特定した最大の欠陥。

### アクティブ / ベンチ / ベンチ切れ
**アクティブ**＝場の前面で殴り合う1体。**ベンチ**＝後方に控える枠。攻撃を受けるのは原則アクティブ
（実測 KO の 130/136 がアクティブ）。**ベンチ切れ**＝アクティブが KO された時に出せる控えが無く
負けること。本プロジェクトの主要な故障モードの1つ（デッキの頭数が律速）。

### デッキ切れ（deck out）
山札が尽きて負けること。**遅滞（stall）型**の勝ち筋でもあり負け筋でもある
（実測: 自分のデッキ切れ負け 2.8% vs 相手のデッキ切れ勝ち 24.8%）。
「山札を戻して回し続ける」札を**リサイクル**と呼ぶ（§39）。

### たね / 進化
**たね**＝手札からそのまま場に出せる出発点のポケモン。**進化**＝たねの上に重ねて強くする。
たねの枚数が場の展開速度を、進化先が打点を決める。展開系の手をコードでは `PLAY` / `EVOLVE` と呼ぶ。

### エネ付与（attach）/ 律速
ワザを撃つには**エネルギーを規定数貯める**必要があり、原則**1ターン1枚**しか付けられない。
このため「撃てるようになるまでの時間」が攻撃の**律速**になる（実測: 主力がアクティブのターンの
66% はエネが足りず撃てない）。どこに付けるかの帯実測を注入したのが `attach_priors`（§59）。

### 引きずり出し（gust）
相手の**ベンチのポケモンを強制的にアクティブへ引きずり出す**効果。壁をどかして
「サイドの重い札」を狙い撃つのに使われる（上位帯は prize 最大を選ぶ・§80）。
コードでは `SWITCH` 系の選択。§79 で**相手指定を自分の場として解決していたバグ**を発見
（修正＝`fix_switch_target`）。

### にげる（retreat）/ 入替札
アクティブをベンチと交代させること。**にげる**はエネを払う（＝損失あり）、**入替札**は
トレーナーで払わず交代できる（＝損失なし）。危険なアクティブを下げるレバーが `evacuate_prize`（§80）で、
**入替札→にげる**の順に試す。

### 壁（wall）
相手の攻撃をほとんど通さない受け役。§81 で「特定の札が特定の相手型に対して被ダメ 17 vs 99」と実測され、
**壁は本物だが、相手は gust でどかしてくる**ことが判明した。

### OHKO / リーサル / `ko_threat`
- **OHKO**（One-Hit KO）: 1回の攻撃で相手を落とすこと。打点の**分水嶺**が勝率を決める
  （実測: 打点 120→OHKO率 0.45 / 170→0.72）。
- **リーサル**: そのターンに勝ち切れる（勝利条件を満たせる）状態。
- **`ko_threat`**（[agents.py](../../src/agents.py)）: 「次に自分のアクティブが落とされる危険度」を
  0〜1 で見積もる関数（§48）。退避レバーの発火条件に使う。

### 帯（band）
レートの近い対戦相手の層。**上位帯**＝リーダーボード上位（`data/replays/others/` に集めた教師プール）。
**帯の回転**＝マッチングする相手の顔ぶれが日ごとに入れ替わる現象で、
**同一ビルドでもレートが ±110 動く**主因（§82）。期間をまたいだレート比較を無効にする。

### カウンター群 / 相性構造
自分の構成に構造的に強い相手デッキの集合。実測では遭遇率 15〜32%・そこでの勝率 0.17〜0.40 に対し
**その他相手は 0.53** で、**平均レートの律速はカウンター群**だった（§70）。
「素の強さ」でなく**相性構造**が上限を作っているという見立て。

### 型（family / F0・F4 …）
デッキを**ロール構成**でまとめた分類（→ [アーキタイプ](#アーキタイプ型と実行度)）。
`pick_opponent_deck` の重みを family へ集約すると**検知率 0.89・中央 6 決定目**で相手の型が当たる（§71）。
ただし「検知後に何を変えるか」は最後まで特定できなかった。

## 実戦 A/B の運用（提出枠・同時対照）

ローカル評価が実戦と逆符号になる（§61）ため、**確定判断は Kaggle 実戦の A/B** で行う。
その運用ノウハウ（§34 が原典）。

### 提出枠（submission slot）/ 旧側・最新側 / 2連アップロード
Kaggle 側で**アクティブな提出は最新2つ**だけ保持され、3つ目を出すと**最古が自動で落ちる**。
つまり枠は「旧側」「最新側」の2つ。狙った2構成を並走させるには**提出順で制御**する
（残したい方を後に出す）。両枠を入れ替えたい時は続けて2本上げる＝**2連アップロード**。

### 単一変数 A/B（§34）
2つの提出物の差分を**1箇所だけ**にして出す作法。本プロジェクトでは
「展開して再帰 diff し、`main.py` の1行のみ差分」まで確認してからアップロードする。
2つ動かすと（例: パック＋マージン）どちらが効いたか永久に分からなくなる＝**交絡**。

### 同時対照（simultaneous control）/ paired A/B
**再提出はレートがリセットされる**（同一 tar でも新規エージェント扱い）。これを逆手に取り、
**両枠を同時に 600 から走らせる**と、レート差・遭遇帯差の交絡が消えた公平な A/B になる（§67）。
逆に「今の枠 vs 先週の枠」は**期間跨ぎ比較**＝帯の回転（±110）に埋もれて無意味（§82）。

### 配線プローブ（wiring probe）
ビルドした提出物を**展開したディレクトリで実際に import し、狙った値がエージェント生成関数まで
到達しているか**を spy で確認する検査。「実装したのに本番で一度も発火していなかった」事故（§49）の再発防止。
⚠️ プローブは `ptcgbot.ismcts` 側に張る——build 時に import が `ptcgbot.*` へ書き換わるため、
素の `ismcts` を patch しても spy が発火しない。

### 床（floor・実績帯）
「触らなければこのくらいは出る」と確認済みの実績レート帯。**接地 floor（安全弁）とは別物**。
同一構成5期の実測は 675/785/640/658/608＝**平均673・レンジ177**で、
**レバーの効果量（±30〜70）はこの幅に完全に埋もれる**＝24h の A/B では原理的に判定できない（§88）。

### 早期中止基準（early stopping criteria）
A/B を最後まで回さず引き上げる条件を**結果を見る前に**決めておくこと
（例: 「15h 時点で初攻撃ターン25超 or 自デッキ切れ負け20%超なら中止」）。→ [事前登録](#事前登録pre-registration)

## 操縦レバーと意思決定の監査

### レバー（lever）
訓練をやり直さず、**推論時のパラメータ1つで挙動を変える**小さな介入。低スペック環境で
「軍拡でなく推論時の工夫」を優先する本プロジェクトの主戦術。既定 off で入れておき、
単一変数 A/B で採否を決める。実装済み: `end_margin` `dev_margin` `select_margin` `bench_first`
`evacuate_prize` `fix_switch_target` `setup_active_rule` `attach_priors` `fetch_priors` `threat_bonus`。

### 逸脱（deviation）とマージン族（`select_margin` / `end_margin` / `dev_margin`）
**逸脱**＝探索（ISMCTS）が heuristic の提案と**違う手を選ぶ**こと。マージンは
「探索が heuristic を**これだけ上回った時にだけ**乗り換えてよい」という閾値で、
上げるほど heuristic に寄る（＝探索の逸脱を抑える）。
- `select_margin`: 全体の既定マージン（[ismcts.py](../../src/ismcts.py)・既定 0.05）。
- `end_margin`: **ターンを終える手（END）へ逸脱する時だけ**要求する非対称マージン（§60）。
- `dev_margin`: **展開手（PLAY/EVOLVE）を蹴って他へ逸脱する時だけ**要求する非対称マージン（§68・不採用）。
- 併用時は**大きい方を採用**（`max`）＝順序非依存で合成できる。
実測: 種別ごと変わる逸脱は **5.9回/戦**で、他の全レバー（0.15〜1.11回/戦）の 5〜40倍の母数（§90）。

### 発火量（回/戦）/ 発火 ≠ 実行
**発火量**＝そのレバーが「選択を実際に変える」回数を1試合あたりで測った値。
投入前に replay で実測し、**効果量の期待値を見積もる**のが本プロジェクトの作法。
ただし**発火（提案）と実行は別物**——MAIN/ATTACK の決定は探索が上書きできるため、
退避レバーは提案 1.06回/戦のうち**採用 30%**しか通らなかった（§83）。
一方**サブ選択は探索に上書きされない**（`_MCTS_SELECT_TYPES` は MAIN と ATTACK のみ）＝発火＝実行。

### サブ選択（sub-select）/ `_generic_select` フォールバック / 先頭取り率
MAIN/ATTACK 以外の細かい選択（何を捨てるか・どれをベンチに出すか・開幕アクティブを誰にするか等）。
[agents.py](../../src/agents.py) `_generic_select` に専用分岐が無いものは末尾の
`return list(range(count))`＝**先頭から必要数を取るだけ**に落ちる。
**先頭取り率 1.000 は「一度も選んでいない」の指紋**（§78）。

### 生存者バイアスを避ける監査の作法（§78）
❌ **自分の仮説の条件で母数を絞る**（例:「HP が半分以下で、にげるが合法な局面」）——
現行方策が作る状態しか観測できず、「機会が無い」という誤った結論が出る。
✅ **同じ決定タイプで上位帯（`others/`）と分布を比べる**。帯の先頭取り率も 1.000 なら
「選ぶ余地の無い決定」＝対照群として機能し、本当に無視してよいと言える
（実測の対照: `SKILL_ORDER` と `IS_FIRST` は帯も 1.000）。
この監査で「1試合あたり約10回の決定を決めていない」ことが判明した。
